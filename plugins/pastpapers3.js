...
async function fetchPastPapers(gradeSlug) {
  const url = `https://govdoc.lk/category/past-papers/${gradeSlug}`;
  const res = await axios.get(url, { headers });
  const $ = cheerio.load(res.data);
  const posts = [];

  $("a.custom-card").each((_, el) => {
    // Filter out "related" or non-paper cards
...
cmd(
  ...
  async (robin, mek, m, { from, q, sender, reply }) => {
    if (!q) return reply("❌ Please provide a grade. Example: `.pastpapers 11`");
    ...
    if (parts.length === 1 && /^\d{1,2}$/.test(parts[0])) {
      ...
      return reply("❌ Invalid grade format. Try `.pastpapers 10` or `.pastpapers grade 11`");
    }

    const posts = await fetchPastPapers(gradeSlug);
    if (!posts.length) return reply(`❌ No past papers found for *${gradeSlug}*`);

    let msg = `📄 *Past Papers: ${gradeSlug.toUpperCase()}*\n────────────────────\n_Reply with number to select paper_\n\n`;
...
cmd(
  ...
  async (robin, mek, m, { from, body, sender, reply }) => {
    ...
    try {
      ...
      // Wait up to 20 seconds for the file
      let fileName;
      for (let i = 0; i < 20; i++) {
